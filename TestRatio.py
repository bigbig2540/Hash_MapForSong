...
def findSong(directory): # หาว่าแต่ละอารมณ์มี คำแต่ละคำอยู่กี่เพลง
    song_dict = dict()
    name_list = os.listdir(directory)
    for name in name_list:
        file = open(directory+name, 'r' , encoding='utf-8')
...
def genDict(directory):
    ...
    all_word = dict()
    for word in total: # ถ้าคำไม่มีใน อารมณ์ไหน ให้เซตเป็น 0
        for x in range(0,4):
    ...
    for word in total: # gen dict >> [มีคำนี้กี่คำในแต่ละ4อารมณ์ , มีคำนี้กี่เพลงในแต่ละ4อารมณ์]
...
for x in num_class:
    num_class_test.append(math.floor(x*ratio))
...
for x in length:
    for y in x:
        ...
        for name in name_list3:
            ...
            for word in ls_text:
                if word in TrainDict.keys():
                    sum_emo_word = sum([float(TrainDict[word][emoi])/TrainDict['Total'][emoi] for  emoi in [0,2,4,6]])
                    sum_emo_song = sum([float(TrainDict[word][emoi]) / TrainDict['Total'][emoi] for emoi in [1, 3, 5, 7]])
                    for idx_f in range(0,8):
                        if(idx_f%2 == 0):
                            f[idx_f] += float(TrainDict[word][idx_f])/TrainDict['Total'][idx_f]/(sum_emo_word)
                        else:
